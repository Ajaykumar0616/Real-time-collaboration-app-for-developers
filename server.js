...
var fdelete = require('rimraf');
fdelete("./server_storage/neeraj@780", function () { console.log("done"); });
//var file = new File("The first upload", 'ajay.txt');
var config = {
  apiKey: "AIzaSyBcHiQh8MeLYyaDiuHQxIR3Z_zhaMkpFQU",
    authDomain: "ippm-8b0b2.firebaseapp.com",
    databaseURL: "https://ippm-8b0b2.firebaseio.com",
    projectId: "ippm-8b0b2",
    storageBucket: "ippm-8b0b2.appspot.com",
    messagingSenderId: "368559540570"
};
firebase.initializeApp(config);
/*var defaultStorage = firebase.database().ref('neeraj');
var cfile = defaultStorage.child('ippm999');
cfile.push("ajay.txt");
cfile.push("ajay1.txt");
cfile.push("ajay2.txt");
...
/*fs.readFile('tslint.json', function(err, data){
...
io.on('connection', function(socket) {
  let previousId;
   ...
   socket.on('forget', function(data){
     ...
     connection.query("select * from register where `gmail`='" + data + "'", function(error, rows, fields){
          if(!!error){
            ...
            var mailOptions = {
              ...
              text: "Hi "+rows[0].username+", Your Password is "+decrypt(rows[0].password)
            };
   ...
   socket.on('createmember', function(data){
     ...
     {
        console.log("true");
        var sql = "INSERT INTO project (`username`, `projectid`, `projectname`, `count`, `size`, `paid`) VALUES ('" + data.user + "', '" + proid + "', '" + data.projectname + "', '" + 1 + "', '" + 0 + "', '"+ 0 +"')";
    ...
    fs.mkdir('./server_storage/'+proid, function(){
      ...
      for(x in data.members){
        ...
        var tomail = data.members[x].substring(data.members[x].indexOf(':') + 2, data.members[x].length)
        var mailOptions = {
          from: 'ippmcmp07@gmail.com',
          to: tomail,
          subject: 'IPPM New Project Collaboration',
          text: "Hi programmer, \nIt is a invitation to collaborate in Project "+data.projectname+".\n For collaboration, login to IPPM or signup click on choose existing \n and enter projectId.\n"+"ProjectId is "+proid
        };
        transporter.sendMail(mailOptions, function(error, info){
          if (error) {
            console.log(error);
...
var project_name = [];
  connection.query("select * from project where `username`='" + data+"'", function(error, rows, fields){
    if (!!error) {
      console.log('error in the Insert query:'+error);
    }
    else {
   ...
   socket.on('tproject', function(data){
    ...
    var proname = data.projectid.substring(data.projectid.indexOf('@')+1, data.projectid.length);
    var sql = "SELECT * from project WHERE `username`='"+data.user+"' AND `projectid`='"+data.projectid+"'";
    connection.query(sql, function(error, rows, fields){
      if (!!error) {
        console.log('error in the Insert query:'+error);
      }
      ...
      if(rows.length == 0){
        var sql = "INSERT INTO project (`username`, `projectid`, `projectname`, `count`, `size`, `paid`) VALUES ('" + data.user + "', '" + data.projectid + "', '" + proname + "', '" + 1 + "', '" + 0 + "', '"+ 0 +"')";
        connection.query(sql, function(error, rows, fields){
          if (!!error) {
            console.log('error in the Insert query:'+error);
          }
    ...
    connection.query("select * from project where `projectid`='" + data.projectid+"' AND `count`=" + 1, function(error, rows, fields){
      if (!!error) {
        console.log('error in the Insert query:'+error);
      }
      else {
  ...
  socket.on('signup', function(data){
    ...
    console.log("email:"+data.email+",password:"+mystr+",usrname:"+data.username);
    //var values = [data.email, mystr, data.username];
    var sql = "INSERT INTO register (`gmail`, `password`, `username`) VALUES ('" + data.email + "', '" + mystr + "', '" + data.username + "')";
    connection.query(sql, function(error, rows, fields){
      if (!!error) {
        console.log('error in the Insert query:'+error);
      }
...
console.log("enter in login:"+data.username);
    connection.query("select * from register where `username`='"+data.username+"'", function(error, rows, fields)
    {
      if(!!error)
      {
...
socket.emit("log", "logkro");
  ...
  socket.on('logout', function(data){
    ...
    for(x in documents){
      ...
      fs.writeFile('./server_storage/'+data.username+'@'+data.folder+'/'+documents[x].id, documents[x].doc);
...
console.log(Object.keys(documents).length);
    ...
    { console.log("documents.length>0");
      connection.query("select * from project where `projectname`='" + data.folder+"' AND `count`=" + 1, function(error, rows, fields){
        if (!!error) {
          console.log('error in the Insert query:'+error);
        }
        else {
        ...
        if(rows.length == 1){
          ...
          fsize('./server_storage/'+data.username+'@'+data.folder, (err, size) => {
            if (err) { throw err; }
            ...
            if(size > 10)
            pay = 1;
            connection.query("update project set `count`="+0+", `size`='"+size.toString()+"', `paid` = "+ pay + " where `projectname`='"+data.folder+"'", function(error, rows, fields){
              if (!!error) {
                console.log('error in the Insert query:'+error);
              }
              else {
    ...
    {
      connection.query("update project set `count`="+0+" where `username`='"+data.username+"'", function(error, rows, fields){
        if (!!error) {
          console.log('error in the Insert query:'+error);
        }
        else {
  ...
  socket.on('delete', function(data){
    fdelete("./server_storage/"+data.user+'@'+data.proid, function () { console.log("done"); });
  });
  ...
  socket.on('createfile', function(data){
    ...
    fs.writeFile('./server_storage/'+data.username+'@'+data.folder+'/'+data.file, '');
